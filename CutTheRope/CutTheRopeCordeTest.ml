...
let calculForceCorde corde balle forcesActuelles =
  ...
  let calcCosAngle = ( fun ()-> if coteAdj<100.0 && coteAdj> -.100.0 then (coteAdj/.corde.longueur)else (coteAdj/.(abs_float coteAdj)))   in
  let cosAngle = (calcCosAngle ()) in
...
let drawCorde corde balle =
  ...
  if corde.longueur**2. < ((p-.u)**2. +. (q-.w)**2.) then (drawLine (cx,cy) (bx,by);)
	...
	let f5 = (fun () -> w -. (a *. (cosh ((p-.x0)/.a)))+.a) in
	let y0 = f5 () in
	let y0alt = f4 () in
	Printf.printf "y0 %f \n" y0 ;
	...
	Printf.printf "TES5T %f %f \n" p (f (p+.1.));
	drawSegment (p+.1.,f (p+.1.)) ;
	Printf.printf "TESENDT";
      )
      else (
...
game {position=(220.0,230.0);vitesse=(0.0,-0.05);masse=1.0;taille=ball};;
